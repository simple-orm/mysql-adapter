  ...
  if(!instance) {
    instance = {
      ...
      bulkInsert: function(models) {
        ...
        } else if(!_.isFunction(models.toArray) && !_.isArray(models)) {
          throw 'bullkInsert can only work on a collection or array of models';
        }
      ...
      bulkRemove: function(models) {
        ...
        if(_.isFunction(models.toArray)) {
          ...
          throw 'bulkRemove can only work on a collection or array of models';
        }
      ...
      find: function(model, criteria, createModelInstance) {
        ...
        adapter.getRow(query.text, query.values).then(function(results) {
          ...
          if(!results) {
            ...
            returnObject = createModelInstance(results, 'loaded');
          }
        ...
        adapter.getAll(query.text, query.values).then(function(results) {
          ...
          if(results.length > 0) {
            ...
            _.forEach(results, function(row) {
              var newObject = createModelInstance(row, 'loaded');
