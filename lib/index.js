...
module.exports = function(connectionObject) {
  return {
    ...
    insert: function(model) {
      ...
      .insert()
      .into(model._table)
      .setFields(model.getInsertDataStoreValues(this._dataConverters))
      .toParam();
    ...
    update: function(model) {
      ...
      .update()
      .table(model._table)
      .setFields(model.getUpdateDataStoreValues(this._dataConverters));
    ...
    _buildSelectQuery: function(model, criteria) {
      ...
      _.forEach(model._selectColumns, function(value) {
        query.field("{0}.{1}".format(model._table, value));
      });
    ...
    _appendWhere: function(query, data) {
      if(data) {
        _.forEach(data, function(value, key) {
          ...
          var sqlValue = value.valueType !== 'field' && _.isString(value.value) ? "'{0}'".format(sqlValue) : sqlValue;

          if(noValuedComparisons.indexOf(comparison) !== -1) {
            query.where('{0} {1}'.format(key, comparison));
          } else if(betweenComparisons.indexOf(comparison) !== -1) {
            query.where('{0} {1} ? AND ?'.format(key, comparison), sqlValue[0], sqlValue[1]);
          } else {
            query.where('{0} {1} ?'.format(key, comparison), sqlValue);
          }
        });
      }
    },
        ...
        _.forEach(data, function(value) {
          ...
          _.forEach(value.on, function(value, key) {
            ...
            if(noValuedComparisons.indexOf(comparison) !== -1) {
              ...
              on += ('{0} {1} ' + sqlValue[0] + ' AND ' + sqlValue[1]).format(key, comparison);
            } else {
